          ...
          >
            <div className="mb-3 flex items-start justify-between gap-2">
              <p className="font-medium text-foreground text-sm leading-snug">
                <span className="mr-1.5 font-bold text-primary">Q{qi + 1}.</span>
                {q.question}
              </p>
              {q.hint && !submitted && (
                <button
            ...
            {q.hint && showHints[qi] && (
              <div className="mb-3 rounded-lg bg-accent/10 p-3 text-xs text-foreground/80">💡 {q.hint}</div>
            )}
            <div className="space-y-2">
              {q.options.map((opt, oi) => {
                const selected = userAnswer === oi;
                const correctOpt = q.correct === oi;
                let optClass = "flex cursor-pointer items-start gap-2.5 rounded-lg border px-3 py-2.5 text-sm transition-all ";
                if (!submitted) {
                  optClass += selected
                    ? "border-primary bg-primary/10 text-foreground font-medium"
                    : "border-border bg-muted/20 text-muted-foreground hover:border-primary/50 hover:bg-muted/40";
                } else {
                  if (correctOpt) optClass += "border-green-500 bg-green-500/10 text-green-700 dark:text-green-400 font-medium";
                  else if (selected && !correctOpt) optClass += "border-red-500 bg-red-500/10 text-red-700 dark:text-red-400";
                  else optClass += "border-border bg-muted/10 text-muted-foreground";
                }
                return (
                  <label key={oi} className={optClass}>
                    <input
                      type="radio" name={`q${qi}`} value={oi} checked={selected} disabled={submitted}
                      onChange={() => setAnswers((p) => ({ ...p, [qi]: oi }))}
                      className="mt-0.5 accent-primary"
                    />
                    <span>{opt}</span>
                    {submitted && correctOpt && <CheckCircle2 className="ml-auto h-4 w-4 text-green-500 shrink-0" />}
                  </label>
                );
              })}
            </div>
      ...
      {!submitted ? (
        ...
        <motion.div
          ...
          className={`rounded-xl border p-5 text-center ${
            score >= 70 ? "border-green-500/30 bg-green-500/5" : "border-yellow-500/30 bg-yellow-500/5"
          }`}
        >
          <p className="text-3xl font-bold text-foreground">{score}/100</p>
          <p className="mt-1 text-sm text-muted-foreground">
            {questions.filter((q, i) => answers[i] === q.correct).length} of {questions.length} correct
          </p>
          <p className="mt-2 font-semibold text-lg">{score >= 70 ? "✅ Passed!" : "❌ Not quite — try again"}</p>
          <div className="mt-4 flex justify-center gap-3">
            <button onClick={handleRetry} className="flex items-center gap-2 rounded-xl border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted">
              <RotateCcw className="h-4 w-4" /> Retry
            </button>
            {score >= 70 && (
              <button onClick={() => onComplete(score)} className="flex items-center gap-2 rounded-xl bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
...
function ExerciseItem({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (s >= masteryThreshold) { setMastered(true); toast.success("🎉 Exercise Mastered!"); }
      else { toast.success(`Score submitted: ${s}/${maxScore}`); }
      setExpanded(false);
    } catch (e: any) {
  ...
  return (
    <div className={`rounded-xl border transition-colors ${ mastered ? "border-green-500/30 bg-green-500/5" : "border-border bg-card" } p-4`}>
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            {mastered && <CheckCircle2 className="h-4 w-4 shrink-0 text-green-500" />}
            <p className="font-medium text-card-foreground truncate">{exercise.title}</p>
          </div>
          <p className="text-xs text-muted-foreground mt-0.5">Max: {maxScore} pts · Mastery: {masteryThreshold} pts required</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {exerciseUri && (
            <a href={exerciseUri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-primary hover:underline">
              <Star className="h-3.5 w-3.5" /> View
            </a>
          )}
          {!mastered && (
          ...
          {mastered && <span className="rounded-full bg-green-500/10 px-2.5 py-1 text-xs font-semibold text-green-600 dark:text-green-400">Mastered ✓</span>}
        </div>
      </div>
      <AnimatePresence>
        {expanded && (
            ...
            <div className="mt-4 border-t border-border pt-4">
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <label className="mb-1 block text-xs font-medium text-muted-foreground">Your Score (0–{maxScore})</label>
                  <input type="number" value={score} onChange={(e) => setScore(e.target.value)} placeholder={`0–${maxScore}`} min={0} max={maxScore} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                </div>
                <div className="w-24">
                  <label className="mb-1 block text-xs font-medium text-muted-foreground">Hints Used</label>
                  <input type="number" value={hintsUsed} onChange={(e) => setHintsUsed(Number(e.target.value))} min={0} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                </div>
                <button onClick={handleSubmit} disabled={submitting || !score} className="flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
...
function EditLessonForm({
  ...
  return (
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mb-6 rounded-xl border border-primary/30 bg-primary/5 p-5 space-y-4">
      ...
      <WalrusUploader label="Quiz" onUploaded={(url) => setNewQuizUri(url)} placeholder="Upload new quiz JSON..." />
...
export default function LessonViewPage() {
  ...
  const currentIdx = sortedLessons.findIndex((l: any) => String(l.id) === lessonId);
  ...
  const handleCompleteWithScore = async (score: number) => {
    ...
    setCompleting(true);
    try {
      await completeLesson(courseId, lessonId, score);
      toast.success("✅ Lesson completed!");
      if (nextLesson) {
        const nextId = typeof nextLesson.id === "string" ? nextLesson.id : String(nextLesson.id);
  ...
  const tabs = [
    ...
    ...(hasExercises ? [{ id: "exercises" as const, label: `💪 Exercises (${exercises.length})` }] : []),
    ...
    <Layout>
      <div className="container mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
          <Link to={`/course/${courseId}`} className="hover:text-foreground transition-colors flex items-center gap-1">
            <ArrowLeft className="h-3.5 w-3.5" />
            {course?.title || "Course"}
          </Link>
          <span>/</span>
          <span className="text-foreground line-clamp-1">{lesson?.title || "Lesson"}</span>
        </div>
        ...
        {lesson ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                <h1 className="font-display text-2xl font-bold text-foreground md:text-3xl">{lesson.title}</h1>
                {lessonCompleted && (
              ...
              {isOwner && !editing && (
                <button onClick={() => setEditing(true)} className="flex shrink-0 items-center gap-1 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-muted transition-colors">
            ...
            {activeTab === "content" && (
              <div className="rounded-xl border border-border bg-card p-6">
                {contentUri ? (
                  ...
                  <p className="text-sm text-muted-foreground">No content uploaded for this lesson.</p>
                )}
              </div>
            )}

            {/* Exercises Tab */}
            {activeTab === "exercises" && exercises && exercises.length > 0 && (
              ...
              <div className="rounded-xl border border-border bg-card p-6">
                ...
                {account && !lessonCompleted ? (
                  ...
                  <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-5 text-center">
                    ...
                    <p className="font-semibold text-foreground">Quiz completed! Lesson marked as done.</p>
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground">Connect your wallet to take the quiz.</p>
                )}
              </div>
            )}
            ...
            {lessonCompleted && (
              <div className="mt-6 rounded-xl border border-primary/20 bg-primary/5 p-4 text-center text-sm flex items-center justify-center gap-2">
                ...
                <span className="font-medium text-foreground">Lesson completed!</span>
                {nextLesson && (
                  <span>
                    {" — "}
                    <Link
            ...
            {account && !lessonCompleted && !hasQuiz && activeTab === "content" && (
              <div className="mt-6 rounded-xl border border-border bg-card p-5">
                <h3 className="mb-1 text-sm font-medium text-foreground">Mark as Complete</h3>
            ...
            <div className="mt-8 flex items-center justify-between gap-4">
              {prevLesson ? (
                ...
                >
                  ...
                  <div className="text-left"><p className="text-xs text-muted-foreground">Previous</p><p className="line-clamp-1">{prevLesson.title}</p></div>
                </Link>
              ) : (
                <Link
                  to={`/course/${courseId}`}
                ...
                >
                  <ChevronLeft className="h-4 w-4" />
                  <div><p className="text-xs text-muted-foreground">Back to</p><p>{course?.title || "Course"}</p></div>
                </Link>
              )}
              {nextLesson ? (
                <Link
                ...
                >
                  <div className="text-right"><p className="text-xs text-muted-foreground">Next</p><p className="line-clamp-1">{nextLesson.title}</p></div>
                ...
                <Link
                  ...
                  className="flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/5 px-4 py-3 text-sm font-medium text-foreground hover:bg-primary/10 transition-colors"
                >
                  <div className="text-right"><p className="text-xs text-muted-foreground">Done!</p><p>Course Overview</p></div>
                  <ChevronRight className="h-4 w-4 text-primary" />
                </Link>
              )}
            </div>
