...
function CourseAnalyticsCard({ course }: { course: any }) {
  const courseId =
    typeof course.id === "string" ? course.id : String(course.id);
  ...
  const studentCompletionByLesson = (lessons || []).map(
    (lesson: any, idx: number) => {
      const lessonId =
        typeof lesson.id === "string" ? lesson.id : String(lesson.id);
      const completions =
        allProgress?.filter(
          (p: any) =>
            String(p.course_id) === courseId &&
            String(p.lesson_id) === lessonId,
        ).length || 0;
  ...
  const maxCompletions = Math.max(
    ...studentCompletionByLesson.map((l) => l.completions),
    1,
  );
  const totalCompletions = studentCompletionByLesson.reduce(
    (s, l) => s + l.completions,
    0,
  );
  const uniqueStudents = new Set(
    allProgress
      ?.filter((p: any) => String(p.course_id) === courseId)
      .map((p: any) => p.student) || [],
  ).size;
        ...
        >
          ...
          {studentCompletionByLesson.length > 0 ? (
            <div className="space-y-2.5">
              {studentCompletionByLesson.map(
                ({ lesson, completions, lessonNumber }) => (
                  <div key={lessonNumber} className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-medium text-foreground ml-2 shrink-0">
                        {completions}{" "}
                        {completions === 1 ? "student" : "students"}
                      </span>
                    </div>
                    <ProgressBar
                      value={completions}
...
export default function AnalyticsPage() {
      ...
      <Layout>
        <div className="container mx-auto px-4 py-20 text-center">
          ...
          <p className="text-muted-foreground">
            Connect to view your course analytics.
          </p>
        </div>
      </Layout>
    );
  ...
  const ownedCourseIds = new Set(
    caps?.map((c: any) => String(c.course_id)) || [],
  );
  const ownedCourses =
    courses?.filter((c: any) => ownedCourseIds.has(String(c.id))) || [];
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
              {[
                ...
                },
                {
                  ...
                  label: "Total Lessons",
                  color: "text-blue-500",
                  value: ownedCourses.reduce(
                    (s: number, c: any) => s + Number(c.lesson_count || 0),
                    0,
                  ),
                },
                {
